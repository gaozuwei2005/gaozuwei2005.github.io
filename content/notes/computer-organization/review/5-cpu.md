...
toc: true
description: 数据通路、常见指令、控制信号的逻辑判断、单周期CPU、多周期CPU的概念、流水线处理器、三种冒险、异常与中断
---
...
![](Pasted-image-20260104175628.png)
...
- **直接编码方式**：微命令字段中每位都代表一个微命令
	- 优点：简单、直观、速度快、并行性好
	- 缺点：指令字长过长，CM 容量很大
	![](Pasted-image-20260105151340.png)
- 字段直接编码方式
...
![](Pasted-image-20260105164620.png)
...
![](Pasted-image-20260105165416.png)
...
| **ALUOp**           | ID      | **EX**  | 指定 ALU 运算类型（加 / 减 / 由 funct 决定）。<br>⚠ 不考具体编码      | 所有需要 ALU 运算的指令        |
...
| **PCWrite / PCSrc** | ID / EX | IF      | 控制 PC 是否更新 & 更新来源。<br>⚠ 只关心“跳不跳”，不考细节             | 跳转或顺序执行时              |
...
增加指令级并行的**技术**：
- **超标量流水线技术（空间并行）**：每个时钟周期并发多条独立指令，以并行操作方式将两条或多条指令编译执行。需要增加功能部件。![](Pasted-image-20260105182442.png)
- **超长指令字技术 / 静态多发射技术（编译期并行）**：编译程序发掘指令间并行性，将多条并行操作指令组成超长指令字，需要多个处理部件

- **超流水线技术（时间并行）**：提高流水线主频以提升流水线性能。![](Pasted-image-20260105183102.png)
